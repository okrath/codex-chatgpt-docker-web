...
| The model says its `turn_token` no longer works and asks you to send another message, but the container has been up the whole time | Luna corrupted its own copy of the live token — it transcribes 37 opaque characters into every call, and a dropped character looks the same to the broker | Recovered automatically: while exactly one turn is live, a copy within two characters of the issued token is bound to that turn (`recoveredTypo=N` in the logs). A refusal now only happens with two live turns or a badly damaged copy, and the model is told to re-read the token rather than to stop |
| The model reports that something is impossible — no browser, no tool, nothing installed | It concluded from its own assumptions, or probed a Windows host with POSIX commands and found nothing | The contract now requires a tool result before any claim about this computer, commands shaped for the host OS, and a look for an installed program before declaring the work impossible. If it still refuses, ask it what command it ran |
| A turn fails with "ChatGPT failed the response itself and offered to regenerate it" | ChatGPT errored on its side mid-response | Nothing to do — the failure is retryable and Codex re-delivers the turn. It is detected by ChatGPT's regenerate control, so it works in any interface language |
| The model says its `turn_token` is invalid/expired/revoked and it cannot reach the workspace | The bridge restarted while that turn was in flight; broker state is in memory, so every outstanding token is dropped (the logs show `liveTurns=0`) | Nothing is broken — send one more message in the same Codex task. A new turn issues a fresh token. No need to restart the task or the container |
| A turn dies with `stream disconnected before completion`, then `Reconnecting` re-runs it | Luna rewrote prose it had already streamed to Codex, usually reacting to a failed tool call, and a Responses delta cannot be retracted | Fixed: the rewrite is dropped instead of failing the turn, and later blocks keep streaming. The log says `keeping the N block(s) Codex received` |
| The same turn runs its commands two or three times, with `Reconnecting` in between | Luna answered without its private rolling checkpoint, so the bridge failed the turn and Codex re-delivered it — re-running every tool call in it | Fixed: the bridge now asks the same chat for the missing checkpoint instead of failing. If you still see it, check `docker compose logs` for `could not recover its rolling checkpoint` |
