...
function skillPacket(sourceMessageIndex: number, body: string, name = "preamble-skill") {
  const sourceText = `<skill name="${name}">${body}</skill>`;
  return {
    name,
    sha256: "0".repeat(64),
    chars: body.length,
...
const SKILL_MARKER = "PREAMBLE-SKILL-MARKER-7Z";

test("A4: an over-budget skill turn delivers the skill body as the last preamble part, after history", () => {
  // A one-part skill body plus enough history that the turn overflows even after the skill is
  // relocated, so history is peeled into earlier parts and the skill lands as the final part.
  const skillBody = `${SKILL_MARKER}\n\n${"skillstep ".repeat(1_000)}`;
  const build = (): { parsed: CodexParsedRequest; skillIndex: number } => {
    const messages: CodexMessage[] = [];
    for (let turn = 0; turn < 40; turn += 1) {
      messages.push(userMessage(`turn ${turn} question ${BIG}`));
      messages.push(assistantMessage(`turn ${turn} answer ${BIG}`));
    }
    messages.push(userMessage("current task: run the invoked skill"));
    messages.push(userMessage(`<skill name="preamble-skill">${skillBody}</skill>`));
    return {
      parsed: {
        modelId: CHATGPT_WEB_LUNA_MODEL_ID,
        stream: true,
      ...
      skillIndex: messages.length - 1,
    };
  };
  const capabilities = { localToolsEnabled: false, solAvailable: false, proAvailable: false };
  const token = undefined; // browser-only Luna: the primary preamble-skill case, no broker token
  const { parsed, skillIndex } = build();
  const packet = skillPacket(skillIndex, skillBody);

  const delivered = withPreloadEnv(
    { CODEX_CHATGPT_WEB_LUNA_PRELOAD: "on", CODEX_CHATGPT_WEB_LUNA_PRELOAD_MAX_PARTS: "50" },
    () => compileLunaBudgetedPrompt(parsed, capabilities, token, { preambleSkillCandidate: packet }),
  );

  expect(delivered.preloadParts).toBeGreaterThan(1); // at least one history part plus the skill part
  expect(delivered.compiled.preamble?.length).toBe(delivered.preloadParts);
  // The skill is the last preamble part; earlier parts are history, not the skill.
  expect(delivered.compiled.preamble!.at(-1)).toContain(SKILL_MARKER);
  expect(delivered.compiled.preamble!.at(-1)).toContain("preamble-skill");
  expect(delivered.compiled.preamble![0]).not.toContain(SKILL_MARKER);
  // The skill body leaves the final message; it carries the reference + earlier-message contract.
  expect(delivered.compiled.text).not.toContain(SKILL_MARKER);
  expect(delivered.compiled.text).toContain("delivered verbatim in an earlier message");
  expect(delivered.compiled.text).not.toContain("codex_tool_call");
  expect(delivered.compiled.text).toContain("current task: run the invoked skill");
  // Every delivered message fits the transport budget; usage sums the parts plus the final.
  for (const part of delivered.compiled.preamble!) {
    expect(estimateTokens(part)).toBeLessThanOrEqual(CHATGPT_LUNA_BROWSER_INPUT_TOKEN_BUDGET);
  }
  expect(estimateCompiledChatGptWebInputTokens(delivered.compiled, CHATGPT_WEB_LUNA_MODEL_ID))
    .toBeLessThanOrEqual(CHATGPT_LUNA_BROWSER_INPUT_TOKEN_BUDGET);
  const partsTokens = delivered.compiled.preamble!.reduce((sum, part) => sum + estimateTokens(part), 0);
  expect(delivered.estimatedTokens).toBeGreaterThan(partsTokens);
}, 20_000);

test("A4: a skill turn under budget stays inline and is byte-identical with or without the candidate", () => {
  const skillBody = `${SKILL_MARKER}\n\n${"small ".repeat(50)}`;
  const packet = skillPacket(1, skillBody);
  const build = (): CodexParsedRequest => ({
    modelId: CHATGPT_WEB_LUNA_MODEL_ID,
    stream: true,
    options: { reasoning: undefined },
    context: {
      systemPrompt: ["You are the model backend."],
      messages: [
        userMessage("current task: run the invoked skill"),
        userMessage(packet.sourceText),
      ],
    },
  } as unknown as CodexParsedRequest);
  const capabilities = { localToolsEnabled: false, solAvailable: false, proAvailable: false };
  ...
  const [withCandidate, without] = withPreloadEnv(
    { CODEX_CHATGPT_WEB_LUNA_PRELOAD: "on" },
    () => [
      compileLunaBudgetedPrompt(build(), capabilities, token, { preambleSkillCandidate: packet }),
      compileLunaBudgetedPrompt(build(), capabilities, token),
    ],
  );

  expect(withCandidate.preloadParts).toBe(0);
  expect(withCandidate.compiled.preamble).toBeUndefined();
  // The skill body stays inline in the single message; no preamble contract is rendered.
  expect(withCandidate.compiled.text).toContain(SKILL_MARKER);
  expect(withCandidate.compiled.text).not.toContain("delivered verbatim in an earlier message");
  // Under budget, the candidate never engages: identical to compiling with no candidate at all.
  expect(withCandidate.compiled.text).toBe(without.compiled.text);
});

test("A4: preamble skill delivery works on a Full localTools Luna turn with a broker token", () => {
  // Full harness (localTools) without danger-full-access: the MCP loader never activates, so the
  // over-budget skill is delivered as a preamble part even though a broker token is present. This
  // exercises the localTools branch of the read-only/token invariants in the compiler.
  const skillBody = `${SKILL_MARKER}\n\n${"skillstep ".repeat(12_000)}`;
  const build = (): { parsed: CodexParsedRequest; skillIndex: number } => {
    const messages: CodexMessage[] = [
      userMessage("current task: run the invoked skill"),
      userMessage(`<skill name="preamble-skill">${skillBody}</skill>`),
    ];
    return {
      parsed: {
        modelId: CHATGPT_WEB_LUNA_MODEL_ID,
  ...
  expect(delivered.preloadParts).toBeGreaterThan(0);
  expect(delivered.compiled.preamble!.join("\n")).toContain(SKILL_MARKER);
  expect(delivered.compiled.text).not.toContain(SKILL_MARKER);
  expect(delivered.compiled.text).toContain("delivered verbatim in an earlier message");
  expect(delivered.compiled.text).toContain("current task: run the invoked skill");
}, 20_000);

test("A4: the skill preamble declines and keeps the skill inline when the part cap is too small", () => {
  const skillBody = `${SKILL_MARKER}\n\n${"skillstep ".repeat(12_000)}`;
  const packet = skillPacket(1, skillBody);
  const build = (): CodexParsedRequest => ({
    modelId: CHATGPT_WEB_LUNA_MODEL_ID,
  ...
  // The oversized skill needs more than one part; a cap of 1 forces the delivery to decline.
  const capped = withPreloadEnv(
    { CODEX_CHATGPT_WEB_LUNA_PRELOAD: "on", CODEX_CHATGPT_WEB_LUNA_PRELOAD_MAX_PARTS: "1" },
    () => compileLunaBudgetedPrompt(build(), capabilities, token, { preambleSkillCandidate: packet }),
  );
  expect(capped.preloadParts).toBe(0);
  expect(capped.compiled.preamble).toBeUndefined();
}, 20_000);
